...
try:
    ...
    print("Error: click not found. Install: pip install click")
    sys.exit(1)
...
@click.option("--scanners", default="sast,secrets,cve", help="Comma-separated list.")
@click.option("--format", "-f", type=click.Choice(["text", "json", "sarif"]), default="text")
@click.option("--output", "-o", help="Output file path.")
@click.option("--config", "-c", help="Config file path.")
@click.option("--severity", help="Min severity: critical, high, medium, low.")
@click.option("--show-details/--no-details", default=False, help="Show remediation.")
def scan(target, scanners, format, output, config, severity, show_details):
    ...
    for name in scanner_names:
        for scanner in scanner_map.get(name, []):
            click.echo(f"Running {scanner.name}...", err=True)
    ...
    if output:
        with open(output, "w") as f:
            f.write(output_text)
        click.echo(f"Results: {output}", err=True)
    else:
...
def sbom(target, format, output):
    ...
    if output:
        click.echo(f"SBOM: {output}", err=True)
    else:
        click.echo(result)
